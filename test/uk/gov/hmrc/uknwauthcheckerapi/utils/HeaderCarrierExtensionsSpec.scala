  ...
  "generateCorrelationId" should {
    "reuse the existing X-Correlation-Id header when present in HeaderCarrier" in {
      ...
      implicit val headerCarrier: HeaderCarrier = new HeaderCarrier(
        otherHeaders = Seq((CustomHeaderNames.xCorrelationId, uuid.toString))
      )

      val correlationId: String = generateCorrelationId()

      correlationId shouldBe uuid.toString
    }

    "generate new X-Correlation-Id header when not present in HeaderCarrier" in {
